...
\newrobustcmd{\fginfoUrl}[0]{\verUrl{5}{http://fginfo.cs.tu-bs.de}}
...
\newenvironment{stundenplan}[6]{
	...
	\newcommand{\Termin}[8]{
		...
		\draw [fill=##8](\wTag * ##3,{yZeit(zeit(##4,##5))}) coordinate (TERMIN) rectangle (##3  * \wTag + \wTag,{yZeit(zeit(##6,##7))});
		\tNode{##1}{##2}
	}
	...
	\newcommand{\abendtermin}[4]{
		...
		\draw [fill=##4](\wTag * ##3,-\hPlan-\hAbendHeader) coordinate (TERMIN) rectangle (##3  * \wTag + \wTag, -\hPlan-\hAbendHeader-\hAbend);
		\tNode{##1}{##2}
	}
